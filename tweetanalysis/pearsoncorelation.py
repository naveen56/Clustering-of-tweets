...
"""
WORD ,it is the pattern which matches with all  words
"""
WORD = re.compile(r'\w+')
"""
...
"""
 @desc:calculate the pearsoncorelation similarity between two strings
...
def calculatescore(text1,text2):
    ...
    var2=0
    var3=0
    for x in range(1,len(intersection)):
        tf1+=list1[x]
        var1+=list1[x]**2
        tf2+=list2[x]
        var2+=list2[x]**2
        var3+=list1[x]*list2[x]

    length=len(intersection)
    den=math.sqrt((length*var1-(tf1**2))*(length*var2-(tf2**2)))

    if den==0:
        return 1

    num=length*var3-tf1*tf2
    result= (num/den)
